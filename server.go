...
func ServeHTTP() {
	...
	argsDTCS = "?jql=project+%3D+CASSANDRA+AND+text~DTCS+ORDER+BY+watchers+DESC&maxResults=100"
	...
	hIssuesDTCS := makeRequest("https://issues.apache.org/jira/rest/api/2/search", argsDTCS)
	...
	http.Handle("/dtcs.json", http.StripPrefix("/dtcs.json", hIssuesDTCS))
